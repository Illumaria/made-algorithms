...
void update_u(uint32_t& u, const uint32_t& result,
              const size_t& i, const uint32_t& n) {
    u = (17 * u + 751 + result + 2 * i) % n + 1;
}


void update_v(uint32_t& v, const uint32_t& result,
              const size_t& i, const uint32_t& n) {
 ...
 public:
    ...
    explicit SparseTable(const uint32_t& n): rows(n) {
        ...
        for (size_t i = 0; i < rows; ++i)
            min_table[i].resize(cols);
    }

    void build() {
        for (size_t i = 1; i < rows; ++i)
            val_vec[i] = update_val(val_vec[i - 1]);

        for (size_t i = 0; i < rows; ++i)
            min_table[i][0] = val_vec[i];

        for (size_t j = 1; j < cols; ++j)
            for (size_t i = 0; i + (1 << j) - 1 < rows; ++i)
                min_table[i][j] =
                    std::min(min_table[i][j - 1],
                             min_table[i + (1 << (j - 1))][j - 1]);
    }
