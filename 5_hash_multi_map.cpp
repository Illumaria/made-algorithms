...
size_t hash(const std::string& key, const size_t& m) {
  size_t result = 0;
  for (const auto& ch : key) result = (A * result + ch) % P;
  return result % m;
}
...
class Set {
 ...
 public:
  ...
  void changeCapacity(bool increase) {
    ...
    for (size_t i = 0; i < cur_capacity; ++i)
      if (!set[i].empty && !set[i].deleted) {
        ...
        while (!newSet[j].empty) j = (j + 1) % capacity;
  ...
  void _insert(const std::string& key) {
    ...
    while (!set[i].empty) {
      if (set[i].key == key && !set[i].deleted) return;
      i = (i + 1) % capacity;
    }
    set[i].key = key;
    set[i].empty = false;
    ...
    if (total_size >= capacity / 2) changeCapacity(true);
  }
  ...
  std::string getAll() const {
    ...
    for (size_t i = 0; i < capacity; ++i)
      if (!set[i].empty && !set[i].deleted) result += set[i].key + " ";

    return result;
  }

  std::string getSize() const { return std::to_string(alive_size); }
};
 ...
 public:
  ...
  std::string _get(const std::string& key) {
    ...
    for (const auto& x : map[i])
      if (x.first == key) return x.second.getSize() + " " + x.second.getAll();
    return "0";
  }
