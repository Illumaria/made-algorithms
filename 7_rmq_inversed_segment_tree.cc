...
class SegmentTree {
 ...
 public:
  ...
  void set(const int32_t& value, const uint32_t& v, const uint32_t& left,
    ...
    if (t[v].left <= right && t[v].right >= left) {
      ...
      if (t[v].left >= left && t[v].right <= right) {
        ...
        t[v].val = std::min(t[2 * v + 1].set_val, t[2 * v + 2].set_val);
        t[v].set_val = t[v].val;
      }
    }
  }

  int32_t get_min(const uint32_t& v, const uint32_t& left,
                  const uint32_t& right) {
  ...
  void final_push() {
    for (size_t i = 0; i < m - 1 + n; ++i) push_set(i);
  }

  int32_t get_val(uint32_t i) const { return t[m - 1 + i].val; }
};
...
int main() {
  ...
  std::sort(q.begin(), q.end(),
            [](auto& left, auto& right) { return left.val < right.val; });
  ...
  if (!consistent) {
    ...
    std::cout << "consistent\n";
    st.final_push();
    for (size_t i = 0; i < n; ++i) std::cout << st.get_val(i) << " ";
  }
