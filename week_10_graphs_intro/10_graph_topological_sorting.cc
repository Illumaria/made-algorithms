...
void dfs(const std::vector<std::vector<uint32_t>>& graph, const uint32_t& from,
         std::vector<uint16_t>& color, std::stack<uint32_t>& top_sort,
         bool& has_cycle) {
  color[from] = 1;
  if (!has_cycle) {
    for (const auto& to : graph[from]) {
      if (color[to] == 0) dfs(graph, to, color, top_sort, has_cycle);
      if (color[to] == 1) {
        has_cycle = true;
        break;
      }
...
int main() {
  ...
  for (uint32_t from = 0; from < n; ++from)
    if (color[from] == 0) dfs(graph, from, color, top_sort, has_cycle);
