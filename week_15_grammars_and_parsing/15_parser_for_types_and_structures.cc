...
std::map<std::string, SizeAlign> types = {
    {"bool", {1, 1}},     {"char", {1, 1}},  {"int", {4, 4}},
    {"long", {8, 8}},     {"short", {2, 2}}, {"signed", {4, 4}},
    {"unsigned", {4, 4}},
};
...
class Lexer {
 public:
  ...
  std::string nextLexeme() { return nextToken().lexeme(); }
 ...
 public:
  ...
  void parse() {
    while (true) {
      ...
      while (!(_curToken.is(Token::Kind::Keyword) ||
        ...
        if (_curToken.is(Token::Kind::EOS)) return;
    ...
    if (_curToken.is(Token::Kind::Keyword)) {
      ...
      if (_curToken.is(Token::Kind::Typename)) {
        ...
        while (true) {
          ...
          if (_curToken.is(Token::Kind::RightCurly)) break;
        ...
        if (types[type_name].size % max_align != 0) {
          types[type_name].size =
              (types[type_name].size / max_align + 1) * max_align;
        }
      }
    }
  }
  ...
  SizeAlign parseType() {
    ...
    while (!_curToken.is(Token::Kind::Typename)) {
      ...
      if (_curToken.is(Token::Kind::RightCurly)) return SizeAlign();
      _curToken = _lexer.nextToken();
    }
