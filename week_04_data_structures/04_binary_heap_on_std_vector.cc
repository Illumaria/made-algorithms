...
class BinaryHeap {
 public:
  BinaryHeap() { operations.push_back(-1); }
  ...
  void siftDown(size_t i) {
    ...
    while (2 * i + 1 < values.size()) {
      ...
      if (right < values.size() && values[right].first < values[left].first)
        j = right;
      if (values[i].first <= values[j].first) break;
      std::swap(operations[values[i].second], operations[values[j].second]);
  ...
  std::pair<int64_t, ssize_t> extractMin() {
    ...
    if (values.size() == 0) return std::make_pair(-1, -1);
