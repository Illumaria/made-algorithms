...
void dfs(const uint16_t from, const std::vector<std::vector<uint16_t>>& graph,
...
int32_t bfs(const uint16_t start, const uint16_t finish,
            const std::vector<std::vector<uint16_t>>& graph,
            const std::vector<Edge>& edges, std::vector<bool>& used,
            std::stack<uint16_t>& path, const uint16_t scale) {
  used[start] = true;
...
uint32_t edmonds_karp_with_scaling(const uint16_t start, const uint16_t finish,
  ...
  uint16_t scale = 1;
  while (scale * 2 <= max_capacity) scale *= 2;
  ...
  for (size_t i = 0; i < edges.size(); ++i)
    if (used[edges[i].from] && !used[edges[i].to]) cut_edges.insert(i / 2);
...
int main() {
  ...
  std::vector<Edge> edges;
  uint16_t from, to;
  int32_t capacity, max_capacity = 0;
  ...
  std::set<uint16_t> cut_edges;
  auto max_flow = edmonds_karp_with_scaling(0, n - 1, graph, edges,
                                            max_capacity, cut_edges);

  std::cout << cut_edges.size() << " " << max_flow << "\n";
  for (const auto& x : cut_edges) std::cout << x + 1 << " ";
  std::cout << "\n";
