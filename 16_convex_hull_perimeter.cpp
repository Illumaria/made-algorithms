...
class ConvexHull {
 ...
 public:
  explicit ConvexHull(std::vector<Point>& points) {
    ...
    _hull.insert(_hull.end(), std::make_move_iterator(lower.begin()),
                 std::make_move_iterator(lower.end() - 1));
    _hull.insert(_hull.end(), std::make_move_iterator(upper.begin()),
                 std::make_move_iterator(upper.end() - 1));
  }

  void build_hull(const std::vector<Point>& points, std::vector<Point>& hull) {
    for (const auto& p : points) {
      while (hull.size() >= 2 &&
             Vector(hull[hull.size() - 2], hull[hull.size() - 1]) %
                     Vector(hull[hull.size() - 2], p) <=
                 0)
        hull.pop_back();
      hull.push_back(p);
    }
  ...
  double perimeter() const {
    ...
    for (size_t i = 0; i < _hull.size(); i++) {
      perimeter += Segment(_hull[i], _hull[(i + 1) % _hull.size()]).length();
    }
