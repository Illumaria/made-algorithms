...
int32_t push_flow(const uint16_t vertex,
    ...
    for (auto& x : graph[vertex])
        if (!used[edges[x].to] && edges[x].flow < edges[x].capacity) {
            int32_t next_flow = std::min(current_flow,
                                         edges[x].capacity - edges[x].flow);
            auto delta = push_flow(edges[x].to, finish, next_flow,
                                   graph, edges, used);
...
uint32_t ford_fulkerson(const uint16_t start,
                        const uint16_t finish,
                        const std::vector<std::vector<uint16_t>>& graph,
                        std::vector<Edge>& edges) {
