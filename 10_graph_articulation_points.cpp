...
void dfs(std::vector<std::vector<uint32_t>>& graph,
         std::set<uint32_t>& articulation_points, std::vector<bool>& used,
         std::vector<uint32_t>& time_in, std::vector<uint32_t>& time_out,
         uint32_t& time, uint32_t from, ssize_t parent = -1) {
  used[from] = true;
  ...
  for (const auto& to : graph[from]) {
    if (to != parent) {
      if (used[to]) {  // There's an alternative route to the "to" vertex
        ...
        dfs(graph, articulation_points, used, time_in, time_out, time, to,
            from);
        ++children_num;
  ...
  if ((parent == -1) && (children_num > 1)) articulation_points.insert(from);
}
...
int main() {
  ...
  for (uint32_t from = 0; from < n; ++from)
    if (!used[from])
      dfs(graph, articulation_points, used, time_in, time_out, time, from);

  // Print the number of articulation points
  // and their corresponding vertices:
  std::cout << articulation_points.size() << "\n";
  for (const auto& x : articulation_points) std::cout << x + 1 << " ";
