...
class SegmentTree {
 private:
    struct Node {
        ...
        Node(): left(0), right(0),
                val(MAX_VALUE), add_val(0), set_val(MAX_VALUE),
                updated(false) {}
    };
    std::vector<Node> t;
    uint32_t n, m;
 ...
 public:
    ...
    void set(const int64_t& value, const uint32_t& v,
        ...
        if (t[v].left <= right && t[v].right >= left) {
            if (t[v].left >= left && t[v].right <= right) {
                ...
                t[v].val =
                    std::min(t[2 * v + 1].set_val + t[2 * v + 1].add_val,
    ...
    void add(const int64_t& value, const uint32_t& v,
             const uint32_t& left, const uint32_t& right) {
        if (t[v].left <= right && t[v].right >= left) {
            push(v);
    ...
    int64_t get_min(const uint32_t& v,
                    const uint32_t& left, const uint32_t& right) {
