...
void init_hashes(const std::vector<std::string>& strings,
    ...
    for (size_t k = 0; k < strings.size(); ++k) {
        ...
        for (size_t i = 1; i < strings[k].size(); ++i)
            hashes[k][i] = (hashes[k][i - 1] * CONST_P + strings[k][i]) %
                           CONST_M;
    }
}
...
std::string f(const std::vector<std::string>& strings,
    ...
    substring_hash_map(substr_hash_map,
                       hashes[min_length_index],
                       strings[min_length_index].size(),
                       powp,
                       length);
    ...
    for (size_t i = 0; i < hashes.size(); ++i)
        substring_hash_set(substr_hash_sets[i],
                           hashes[i],
                           strings[i].size(),
                           powp,
                           length);
    ...
    for (size_t i = 1; i < hashes.size(); ++i) {
        ...
        std::set_intersection(result.begin(),
                              result.end(),
                              substr_hash_sets[i].begin(),
                              substr_hash_sets[i].end(),
