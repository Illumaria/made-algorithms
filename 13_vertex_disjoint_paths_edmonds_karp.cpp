...
std::vector<Point> build_moves(const uint16_t map_x,
                               const uint16_t map_y,
                               const Point& v) {
    std::vector<Point> result;

    for (const auto& move : moves)
        if (-1 < v.x + move.x && v.x + move.x < map_x &&
            -1 < v.y + move.y && v.y + move.y < map_y)
...
int32_t bfs(const Point& start,
    ...
    if (finish_reached) {
        ...
        while (!(current.x == start.x && current.y == start.y)) {
            min_flow =
                std::min(min_flow,
                         edges[ancestors[current.x][current.y]].capacity -
...
uint64_t edmonds_karp(const Point& start,
                      const Point& finish,
    ...
    for (size_t i = 0; i < edges.size(); ++i)
        if (used[edges[i].from.x][edges[i].from.y] &&
            !used[edges[i].to.x][edges[i].to.y] &&
            edges[i].flow == 1)
...
int main() {
    ...
    */
    ...
    auto max_flow = edmonds_karp(start,
                                 finish,
                                 graph,
                                 edges,
                                 2 * n,
