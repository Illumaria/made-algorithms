...
void dfs(std::vector<std::vector<uint32_t>>& graph, const uint32_t& from,
         std::vector<bool>& used, std::stack<uint32_t>& top_sort) {
  used[from] = true;
  for (const auto& to : graph[from])
    if (!used[to]) dfs(graph, to, used, top_sort);
  top_sort.push(from);
}

void dfs_inv(std::vector<std::vector<uint32_t>>& graph_inv, uint32_t from,
             std::vector<uint32_t>& scc, uint32_t current_scc) {
  scc[from] = current_scc;
  for (const auto& to : graph_inv[from])
    if (scc[to] == 0) dfs_inv(graph_inv, to, scc, current_scc);
}
...
int main() {
  ...
  for (uint32_t from = 0; from < n; ++from)
    if (!used[from]) dfs(graph, from, used, top_sort);
  ...
  for (uint32_t from = 0; from < n; ++from)
    for (const auto& to : graph[from])
      if (scc[from] != scc[to]) edges_num.insert({scc[from], scc[to]});
