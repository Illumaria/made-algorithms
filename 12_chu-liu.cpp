...
void dfs(const std::vector<std::vector<Edge>>& graph,
...
void condensation_dfs(const std::vector<std::vector<Edge>>& graph,
    ...
    for (const auto& edge : graph[from])
        if (!used[edge.to])
            condensation_dfs(graph, edge.to, used, top_sort);
    top_sort.push(from);
}
...
std::pair<std::vector<uint16_t>, uint16_t> condensation(
    ...
    for (uint16_t from = 0; from < graph.size(); ++from)
        if (!used[from])
            condensation_dfs(graph, from, used, top_sort);
...
int64_t findMST(const std::vector<std::vector<Edge>>& graph,
                const uint16_t n,
                const uint16_t root) {
    int64_t res = 0;
    ...
    if (all_vertices_reachable(zeroEdges, root))
        return res;
        ...
        for (const auto& edge : graph[from])
            if (newComponents[edge.from] != newComponents[edge.to])
                newEdges[newComponents[edge.from] - 1].push_back({
                    newComponents[edge.from] - 1,
                    newComponents[edge.to] - 1,
