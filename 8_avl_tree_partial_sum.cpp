...
class AVLTree {
 ...
 public:
  ...
  void fixSum(Node*& v) {
    v->sum = 0;
    if (v->left != nullptr) v->sum += v->left->sum + v->left->key;
    if (v->right != nullptr) v->sum += v->right->sum + v->right->key;
  }
  ...
  int64_t sum(const Node* v, const int64_t& left, const int64_t& right) const {
    if (v == nullptr) return 0;

    if (v->key > right) return sum(v->left, left, right);

    if (v->key < left) return sum(v->right, left, right);

    if (v->left == nullptr && v->right == nullptr) return v->key;

    if (v->min >= left && v->max <= right) return v->sum + v->key;
